...
# XXD Panel 160｜خطوط حبر حرة وألوان مائية خفيفة
...
وُلّدت هذه النماذج الثمانية بواسطة Panel 160، كل منها بمحاولة واحدة ومن صورة مصدر مختلفة مع نص إنجليزي قصير. ضُبطت الأبعاد إلى 1536×864 بتقسيم متساوٍ يميناً ويساراً و1152×1536 بتقسيم متساوٍ أعلى وأسفل، وتم التحقق من تنظيف البيانات الوصفية. هذه نتائج فعلية وليست تأكيداً لاجتياز جميع متطلبات الأسلوب.

الانحرافات الملحوظة: بعض العناصر والمشاهد كبيرة مع تفاصيل بيئية زائدة، وبعض ألوان المصدر أضعف. حُفظت نتائج المحاولة الواحدة دون توليد إضافي.
...
خطوط حبر حرة ومترددة وغسلات شفافة قليلة، مع 2–4 ألوان من الأصل وبؤرة صغيرة وفراغ كبير.
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 160 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-160`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-160 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-160-prompt.en.md) أو [بالصينية](references/xxd-panel-160-prompt.zh-CN.md).
