...
異なる元写真から Panel 160 で各1回生成した8点です。短い英文を使用し、横1536×864・左右均等、縦1152×1536・上下均等に整え、メタデータ清掃を検証しました。実際のモデル出力を残しており、全要件への適合を保証するものではありません。

確認した偏差：被写体や場面が大きく、環境の細部が残りすぎる例や、元写真の色が弱まる例があります。追加生成せず、初回結果を残しています。
...
自由な墨線、探り線、少量の透明な淡彩。原写真から2–4色を抽出し、小さな焦点と大きな余白を作ります。

## 4つの出力モード

- `top-bottom`：全幅の上下2領域のみ。実写を上、デザインを下に置き、各50%。
- `left-right`：全高の左右2領域のみ。実写を左、デザインを右に置き、各50%。上下構成へ回転しません。
- `design-only`：全画面を Panel 160 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-160` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-160 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-160-prompt.en.md)／[中国語](references/xxd-panel-160-prompt.zh-CN.md)を参照してください。
